...
exports.getAllDoorThreshold = async (req, res) => {
  try {
    const [
      ...
      DoorThreshold.find().select(
        "_id frameTypeOptionId thresholdName thresholdValue status"
      ),
    ]);
                  ...
                  options: frameTypeOptions
                    ...
                    .map((option) => ({
                      ...
                      thresholds: thresholds
                        .filter(
                          (threshold) =>
                            threshold.frameTypeOptionId.toString() ===
                            option._id.toString()
                        )
                        .map((threshold) => ({
                          _id: threshold._id,
...
exports.updateDoorThreshold = async (req, res) => {
  try {
    const updatedThreshold = await DoorThreshold.findByIdAndUpdate(
      req.params.id,
      req.body,
      {
    ...
    return res.status(200).json({
      success: true,
      message: "Door threshold updated successfully",
